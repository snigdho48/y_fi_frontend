...
| `/admin/login` | AdminLogin | Theme Studio (local / dashboard-backend only; **not** on production domain) |
| `/admin/theme` | ThemeEditor | Theme Studio (local only) |

On production, nginx sends `/admin/` to **Django admin**. SPA marketing routes are `/`, `/contact`, `/privacy`.
